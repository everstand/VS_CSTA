...
EXPERIMENT_MODULES_ENABLED = (
    config.use_multi_annot_loss
    or (config.use_rank_loss and config.rank_loss_weight > 0)
    or (config.use_shot_loss and config.shot_loss_weight > 0)
)

if EXPERIMENT_MODULES_ENABLED:
...
for dataset in tqdm(config.datasets,total=len(config.datasets),ncols=70,leave=True,position=0):
    ...
    dataloaders = create_dataloader(dataset,config=config) if EXPERIMENT_MODULES_ENABLED else create_dataloader(dataset)
    for split_id,(train_loader,test_loader) in tqdm(enumerate(dataloaders),total=5,ncols=70,leave=False,position=1,desc=dataset):
        model = set_model(
            model_name=config.model_name,
            Scale=config.Scale,
            Softmax_axis=config.Softmax_axis,
        ...
        ma_criterion = None
        if EXPERIMENT_MODULES_ENABLED and config.use_multi_annot_loss:
            ma_criterion = MultiAnnotationLoss(
                precision_ema=config.ma_precision_ema,
                precision_min=config.ma_precision_min,
                precision_max=config.ma_precision_max,
                warmup_epochs=config.ma_precision_warmup
            )
        ...
        for epoch in tqdm(range(config.epochs),total=config.epochs,ncols=70,leave=False,position=2,desc=f'Split{split_id+1}'):
            ...
            if EXPERIMENT_MODULES_ENABLED:
                for sample in tqdm(train_loader,ncols=70,leave=False,position=3,desc=f'Epoch{epoch+1}_TRAIN'):
                    feature,gtscore,dataset_name,video_num = sample[:4]
                    cursor = 4
                    annot_scores = None
                    shot_index = None
                    if config.use_multi_annot_loss:
                        annot_scores = sample[cursor].to(config.device)
                        cursor += 1
                    if config.use_shot_loss:
                        shot_index = sample[cursor].to(config.device)

                    feature = feature.to(config.device)
                    gtscore = gtscore.to(config.device)
                    output = model(feature)

                    mse_loss = criterion(output,gtscore)
                    if ma_criterion is not None:
                        ma_loss = ma_criterion(output,gtscore,annot_scores,dataset_name,epoch=epoch)
                        ma_loss_weight = resolve_dataset_loss_weight(
                            dataset_name,
                            config.ma_loss_weight,
                            summe_weight=config.ma_loss_weight_summe,
                            tvsum_weight=config.ma_loss_weight_tvsum,
                        )
                        loss = mix_losses(mse_loss,ma_loss,ma_loss_weight)
                    else:
                        loss = mse_loss
                    if config.use_rank_loss and config.rank_loss_weight>0:
                        loss = loss + config.rank_loss_weight * pairwise_rank_loss(output,gtscore,min_target_diff=config.rank_loss_min_diff)
                    if config.use_shot_loss and config.shot_loss_weight>0:
                        loss = loss + config.shot_loss_weight * shot_pairwise_rank_loss(output,gtscore,shot_index,min_target_diff=config.shot_loss_min_diff)
            ...
            with torch.no_grad():
                if EXPERIMENT_MODULES_ENABLED:
                    for sample in tqdm(test_loader,ncols=70,leave=False,position=3,desc=f'Epoch{epoch+1}_TEST'):
                        feature,gtscore,dataset_name,video_num = sample[:4]
                        feature = feature.to(config.device)
                        gtscore = gtscore.to(config.device)
                        output = model(feature)
        ...
        if ma_criterion is not None:
            print(f'[MA-CSTA] {dataset} split{split_id+1} precision={ma_criterion.summary()}')
